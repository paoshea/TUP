...
exports.createEvaluation = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const evaluation = await services_1.evaluationService.createEvaluation(req.body, evaluatorId);
...
exports.updateEvaluation = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
    const { evaluationId } = req.params;
    const evaluation = await services_1.evaluationService.updateEvaluation(evaluationId, req.body, userId);
    res.json({ success: true, data: evaluation });
});
exports.deleteEvaluation = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
    const { evaluationId } = req.params;
    await services_1.evaluationService.deleteEvaluation(evaluationId, userId);
